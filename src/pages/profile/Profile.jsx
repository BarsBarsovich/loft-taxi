...
function Profile(props) {
    ...
    const renderFragment = () => {
        if (props.profileChanged) {
            return <>
                <p className="title">Профиль</p>
                <span className="profile-subtitle mb28">Платёжные данные обновлены. Теперь вы можете заказывать такси.</span>
                <a href="/map">
                    <Button title='Перейти на карту'/>
                </a>
            </>
        } else {
            return <>
                        ...
                        <div className="profile__fio">
                            ...
                            <div className="profile__card-dates">
                                <label className="login__label mr35" data-testid='input'>
                                    ...
                                    <input defaultValue={props.profileInfo.expiryDate} type="number"
                                           placeholder='MM/YY' className='login__input mr35'
...
export default connect(
    state => ({
        ...
        profileChanged: state.profile.profileChanged
    }), {
        LOGOUT_ACTION,
        setProfile,
        setProfileAction,
